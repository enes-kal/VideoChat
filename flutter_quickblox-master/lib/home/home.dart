...
import 'package:quickblox_app/chat/chatscreen.dart';
...
class _HomeState extends State<Home> {
  ...
  _connectToChat(QBUser user) async {
    try {
      await QB.chat.connect(user.id, "12345678");
    } catch (e) {
      print("error");
      print(e);
    }
  ...
  _createChatOneByOne() async {
    try {
      QBDialog createdDialog = await QB.chat.createDialog([123729719, 123730736], "Private Chat ea", dialogType: QBChatDialogTypes.CHAT);
  ...
  _sendChat() async {
    try {
      await QB.chat.sendMessage("5f9ae221a0eb47029fe9baf5", body: "Test EA 1", markable: false, saveToHistory: true);
    } catch (e) {
      print("send caht");
      print(e.toString());
    }
  ...
  _getHistory() async {
    try {
      List<QBMessage> messages = await QB.chat.getDialogMessages("5f9ae221a0eb47029fe9baf5", markAsRead: true);
  ...
  void chatscreen( )
  {
   ...
   Navigator.push(
    context,
    MaterialPageRoute(builder: (context) => ChatScreen('5f9ae221a0eb47029fe9baf5')),
  );
  }
